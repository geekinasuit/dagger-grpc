 ...
 * logging method is implemented by [APTNode] then the underlying element is plumbed through. (This
 * should be the only non-test usage pattern, but the contrary condition is captured, and then no
 * element is offered).
 */
class APTLogger(private val messager: Messager) : KSPLogger {
  override fun error(message: String, symbol: KSNode?) {
    val element = if (symbol is APTNode) symbol.element else null
